...
async def parse_and_save(xml_bytes: bytes):
    ...
    async with session_factory() as session:
        for table in (PersonRelationTable, PersonEventTable, PersonTable, EventTable, PlaceTable, FamilyTable):
        ...
        family_map: dict[str, FamilyTable] = {}
            ...
            for doc_el in documents_root.findall("document"):
                ...
                for detail_el in details_el.findall("detail"):
                    ...
                    if pid and pid in persons_in_xml:
                        person = await session.get(PersonTable, pid)
        ...
        for p1, p2, rcode, rlabel in [
            ...
            exists = await session.execute(
                select(PersonRelationTable).where(
                    PersonRelationTable.person_id == p1,
                    PersonRelationTable.related_person_id == p2,
                )
            )
            if not exists.first():
                session.add(PersonRelationTable(
