...
from src.api.dependencies import UserDep, AdminVerifiedDep
...
async def load_xml_file(session: sessionDep, user_id: AdminVerifiedDep, file: UploadFile):
    await load_xml_service(session, file)
    return {"status": "ok"}


@router.post("/load_gedcom")
async def load_gedcom(session: sessionDep, user_id: AdminVerifiedDep, file: UploadFile):
    msg = await load_gedcom_service(session, file)
    return {"status": "ok", "message": msg}


@router.post("/load_photos")
async def load_photos(user_id: AdminVerifiedDep, file: UploadFile):
    count = await load_photos_service(file)
    return {"status": "ok", "count_files": count}


@router.post("/change_account")
async def change_admin_account(user_id: AdminVerifiedDep, body: ChangeAccountSchema):
