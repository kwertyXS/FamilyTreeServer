...
from src.api.dependencies import authDep, UserDep
...
async def get_tree(session: sessionDep, token: authDep):
    """Полное дерево: все люди + связи для отрисовки на фронте."""
    return await get_tree_service(session, token.credentials)


@router.get("/persons")
async def get_persons(session: sessionDep, user_id: UserDep):
    """Список всех людей."""
    return await get_persons_service(session)


@router.get("/persons/{person_id}", response_model=PersonSchemaOut)
async def get_person(session: sessionDep, user_id: UserDep, person_id: str):
...
@router.get("/events")
async def get_events(session: sessionDep, user_id: UserDep):
